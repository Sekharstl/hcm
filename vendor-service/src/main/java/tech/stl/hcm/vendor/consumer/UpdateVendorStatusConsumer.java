...
public class UpdateVendorStatusConsumer extends BaseTransactionConsumer<VendorStatusDTO> {
    private final VendorStatusService vendorStatusService;

    public UpdateVendorStatusConsumer(ObjectMapper objectMapper, TransactionUpdateService transactionUpdateService, 
    ...
    protected String getEntityName() {
        return "Vendor Status Update";
    }

    @Override
    protected Object processEntity(VendorStatusDTO status, UUID transactionId) {
        vendorStatusService.updateVendorStatus(status);
        return status;
    }
