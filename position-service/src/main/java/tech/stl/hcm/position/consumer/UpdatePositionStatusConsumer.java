...
@Component
@TopicListener(
        topic = "${positionstatus.kafka.topic.update}",
        groupId = "${positionstatus.kafka.group-id}",
        valueType = Map.class,
        enableProperty = "positionstatus.kafka.enable"
)
public class UpdatePositionStatusConsumer extends BaseTransactionConsumer<PositionStatusDTO> {
    private final PositionStatusService positionStatusService;
    
    public UpdatePositionStatusConsumer(PositionStatusService positionStatusService, 
    ...
    protected Object processEntity(PositionStatusDTO status, UUID transactionId) {
        return positionStatusService.updatePositionStatus(status);
    }
