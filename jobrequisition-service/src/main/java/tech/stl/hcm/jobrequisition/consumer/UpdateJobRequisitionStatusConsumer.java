...
@Component
@TopicListener(
        topic = "${jobrequisitionstatus.kafka.topic.update}",
        groupId = "${jobrequisitionstatus.kafka.group-id}",
        valueType = Map.class,
        enableProperty = "jobrequisitionstatus.kafka.enable"
)
public class UpdateJobRequisitionStatusConsumer extends BaseTransactionConsumer<JobRequisitionStatusDTO> {
    private final JobRequisitionStatusService jobRequisitionStatusService;
    
    public UpdateJobRequisitionStatusConsumer(JobRequisitionStatusService jobRequisitionStatusService, 
    ...
    protected Object processEntity(JobRequisitionStatusDTO status, UUID transactionId) {
        return jobRequisitionStatusService.updateJobRequisitionStatus(status);
    }
