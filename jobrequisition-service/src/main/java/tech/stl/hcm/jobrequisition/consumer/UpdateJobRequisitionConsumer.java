...
@Component
@TopicListener(
        topic = "${jobrequisition.kafka.topic.update}",
        groupId = "${jobrequisition.kafka.group-id}",
        valueType = java.util.Map.class,
        enableProperty = "jobrequisition.kafka.enable"
)
public class UpdateJobRequisitionConsumer extends BaseTransactionConsumer<JobRequisitionDTO> {
    private final JobRequisitionService jobRequisitionService;

    public UpdateJobRequisitionConsumer(ObjectMapper objectMapper, TransactionUpdateService transactionUpdateService, JobRequisitionService jobRequisitionService) {
    ...
    protected String getEntityName() {
        return "Job Requisition Update";
    }

    @Override
    protected Object processEntity(JobRequisitionDTO jobRequisition, UUID transactionId) {
        jobRequisitionService.updateJobRequisition(jobRequisition);
